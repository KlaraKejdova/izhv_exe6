...
#region Internal
    ...
    private static Vector2 WINDOW_DIMENSION = new Vector2(256.0f, 300.0f);
    ...
    private void MainWindowUI(int windowId)
    {
        ...
        { // Main Window Area
            ...
            GUILayout.BeginVertical();
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    GUILayout.Label("Master Mute: ", GUILayout.Width(WINDOW_DIMENSION.x / 4.0f));
                    var muted = SoundManager.Instance.masterMuted;
                    muted = GUILayout.Toggle(muted, muted ? "Muted" : "Unmuted");
                    if (GUI.changed)
                    {
